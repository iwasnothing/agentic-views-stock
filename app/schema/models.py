...
# Individual dimension models for split processing
class BusinessModelOutput(BaseModel):
    """Output for business model dimension."""
    business_model: str = Field(description="Description of the business model in 100 words")


class WhatTheySellOutput(BaseModel):
    """Output for what they sell and who buys dimension."""
...
class HowTheyMakeMoneyOutput(BaseModel):
    """Output for how they make money dimension."""
    how_they_make_money: str = Field(description="Revenue model, pricing logic, and key revenue segments")


class RevenueQualityOutput(BaseModel):
    """Output for revenue quality dimension."""
...
class CostStructureOutput(BaseModel):
    """Output for cost structure dimension."""
    cost_structure: str = Field(description="Major cost drivers, gross/operating margins, and scalability")


class CapitalIntensityOutput(BaseModel):
    """Output for capital intensity dimension."""
...
class GrowthDriversOutput(BaseModel):
    """Output for growth drivers dimension."""
    growth_drivers: str = Field(description="Main levers for revenue growth — structural or cyclical")


class CompetitiveEdgeOutput(BaseModel):
    """Output for competitive edge dimension."""
